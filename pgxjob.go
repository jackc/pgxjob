...
type circularJobQueue struct {
	jobs   []*Job
	rp, wp int  // read and write positions
	isFull bool // when rp and wp are equal the queue is either full or empty.
}

func newCircularJobQueue(size int) *circularJobQueue {
	if size < 1 {
		panic("pgxjob: size must be greater than 0")
	}
	return &circularJobQueue{jobs: make([]*Job, size)}
}

// Enqueue adds a job to the queue. Panics if the queue is full.
func (q *circularJobQueue) Enqueue(job *Job) {
	if q.isFull {
		panic("pgxjob: cannot enqueue into full queue")
	}
...
// Dequeue removes a job from the queue and returns it. Panics if the queue is empty.
func (q *circularJobQueue) Dequeue() *Job {
	if q.IsEmpty() {
		panic("pgxjob: cannot dequeue from empty queue")
	}

	job := q.jobs[q.rp]
	q.jobs[q.rp] = nil // avoid pinning *Job in memory
	q.rp++
	if q.rp == len(q.jobs) {
		q.rp = 0
	}
...
func (q *circularJobQueue) Len() int {
	if q.isFull {
		return len(q.jobs)
	}

	if q.wp < q.rp {
		return q.wp + len(q.jobs) - q.rp
	}

	return q.wp - q.rp
}

// Cap returns the maximum number of jobs that can be in the queue.
func (q *circularJobQueue) Cap() int {
	return len(q.jobs)
}

func (q *circularJobQueue) IsEmpty() bool {
	return q.rp == q.wp && !q.isFull
}

func (q *circularJobQueue) IsFull() bool {
	return q.isFull
}
